        ...
        for sas in ass:
            ...
            for check in ansl:
                if a == check:
                    ...
                    for x in assc:
                        x = x.lower()
                        if x == sas and x not in com and x not in keywords:
                            keywords.append(x)
def get_synonyms(keywords):
    synonyms = []
    for word in keywords:  # Iterate over a copy of keywords
        for syn in wordnet.synsets(word):
            for lemma in syn.lemmas():
                synonym = lemma.name()
                synonym = ''.join(char if char.isalnum() else ' ' for char in synonym)
                synonym = synonym.lower().strip()   # Convert to lowercase and remove leading/trailing spaces
...
class Report(tk.Frame):
    def __init__(self):
        ...
        for t in text:
            t = t.lower()
            if t in keywords and t not in repeat:
                value = keywords.index(t)
                if value >= 2:
                    kt = kt + 0.05
                elif value == 0:
                    kt = kt + 0.1
        ...
        for i in range(0, len(repeat) - 1):
            if keywords.index(repeat[i]) < keywords.index(repeat[i + 1]) and repeat[i] not in check:
